...
(defmethod dump-on-db java.util.HashMap
  ...
  (let [entries (.get source "urls")
    ...
    (jdbc/insert-multi! db-spec :leads [:category_id :state_id :city_id :lead_url :lead_phone :seo_value :ppc_value]
                        (persistent! (reduce (fn [results lead-url]
                                               (let [lead-map (.get source lead-url)
                                                     ...
                                                     phone (if-bound phone (and (not= nil :binding)
                                                                     ...
                                                                     phone)]
                                                 (conj! results [category-id state-id city-id lead-url phone seo ppc])))
                                             (transient [])
                                             entries)))))
